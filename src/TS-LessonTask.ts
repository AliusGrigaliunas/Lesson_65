...
const numbers: number[] = [1, 9, 9, 8, 1, 9, 8, 10];
...
// Užduoties sprendimas ir užduoties sprendimo atspausdinimas su duomenimis
const putIndexInLines = (a:number[]) => {
  const indexes = a.map((_num, index) => `${index}`);
  return indexes;
};
{
...
{
  // Užduoties sprendimas ir užduoties sprendimo atspausdinimas su duomenimis
  const getValueOfArray = (a:number[]) => a.map((el) => console.log(el));
  getValueOfArray(numbers);
}
console.groupEnd();
